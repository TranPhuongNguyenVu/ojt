...
describe("splitSeatsInRow", () => {
  it("splits a wide row (>8 seats) with a fixed left/right cut at column 8", () => {
    const rowSeats = Array.from({ length: 10 }, (_, i) => ({ seatColumn: String(i + 1) }));
    const { leftSeats, rightSeats } = splitSeatsInRow(rowSeats);
    expect(leftSeats.map((s) => s.seatColumn)).toEqual(["1", "2", "3", "4", "5", "6", "7", "8"]);
    expect(rightSeats.map((s) => s.seatColumn)).toEqual(["9", "10"]);
  });

  it("splits a short row (<=8 seats) evenly down the middle", () => {
    const rowSeats = Array.from({ length: 6 }, (_, i) => ({ seatColumn: String(i + 1) }));
    const { leftSeats, rightSeats } = splitSeatsInRow(rowSeats);
    expect(leftSeats.map((s) => s.seatColumn)).toEqual(["1", "2", "3"]);
    expect(rightSeats.map((s) => s.seatColumn)).toEqual(["4", "5", "6"]);
  });

  it("handles an empty row without throwing", () => {
    const { leftSeats, rightSeats } = splitSeatsInRow([]);
    expect(leftSeats).toEqual([]);
    expect(rightSeats).toEqual([]);
  });
});
