...
export const splitSeatsInRow = (rowSeats) => {
  const sorted = [...rowSeats].sort(
    (a, b) => getSeatColumnOrder(a.seatColumn) - getSeatColumnOrder(b.seatColumn)
  );
  const maxOrder = Math.max(
    ...sorted.map((seat) => getSeatColumnOrder(seat.seatColumn)),
    0
  );
  const splitAt = maxOrder <= 8 ? Math.ceil(maxOrder / 2) : 8;

  return {
    leftSeats: sorted.filter((seat) => getSeatColumnOrder(seat.seatColumn) <= splitAt),
    rightSeats: sorted.filter((seat) => getSeatColumnOrder(seat.seatColumn) > splitAt),
  };
};
