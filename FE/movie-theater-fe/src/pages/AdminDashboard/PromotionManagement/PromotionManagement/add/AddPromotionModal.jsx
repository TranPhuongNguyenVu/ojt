...
const AddPromotionModal = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await PromotionService.create(buildPromotionPayload(formData));
      onSuccess();
      onClose();
    } catch (error) {
      setErrorMessage(
        getApiErrorMessage(error, "Không thể thêm khuyến mãi.")
      );
    } finally {
  ...
  return (
    <PromotionModalLayout title="Thêm khuyến mãi mới" onClose={onClose}>
