...
const CinemaRoomDetail = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 min-h-[400px]">
        ...
        <button
          ...
          className="flex items-center gap-2 border border-slate-200 bg-white text-slate-700 text-sm font-bold px-4 py-2 rounded-lg hover:border-slate-400 transition-colors cursor-pointer"
        >
          <ArrowLeft size={16} />
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 font-sans -m-8 md:-m-10">
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold text-slate-800">{CINEMA_ROOM_LABELS.setupModalTitle}</h2>
        </div>
        <div className="flex items-center gap-3">
          <button
            disabled={isSaving}
            onClick={() => setIsEditingInfo(true)}
            className="flex items-center gap-1.5 border border-slate-200 hover:border-[#C00000] hover:text-[#C00000] disabled:bg-slate-100 bg-white text-slate-700 text-sm font-bold px-4 py-2 rounded-lg transition-colors cursor-pointer"
          >
            <Edit size={16} />
          ...
          <button
            ...
            className="flex items-center gap-1.5 border border-slate-200 hover:border-blue-600 hover:text-blue-600 disabled:bg-slate-100 bg-white text-slate-700 text-sm font-bold px-4 py-2 rounded-lg transition-colors cursor-pointer"
          >
            <Grid size={16} />
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-6">
        {errorMessage && (
          <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 font-bold flex items-center gap-2">
            <AlertCircle size={18} />
            {errorMessage}
          </div>
        )}
        ...
        {successMessage && (
          <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800 font-bold flex items-center gap-2">
            <Check size={18} />
            {successMessage}
          </div>
        )}

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-wrap justify-between items-center gap-6">
              ...
              <span className="flex items-center gap-1">
                ...
                {room.formats && room.formats.length > 0
                  ? room.formats.map((f) => (
                      <span
                        key={f.versionId}
                        className="ml-1 px-2 py-0.5 rounded bg-slate-100 border border-slate-300 text-slate-700"
                      >
                        {f.versionName}
                      </span>
                    ))
                  : <span className="ml-1 text-slate-400 italic">Chưa có định dạng</span>
                }
              </span>
              <span>•</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-8">
            <div className="space-y-2">
              <div className="w-full h-1.5 bg-slate-200 rounded-full shadow-sm"></div>
              <p className="text-[10px] font-black text-slate-400 text-center tracking-[0.4em] uppercase">
                MÀN HÌNH CHIẾU
              </p>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
              <h3 className="text-xs font-black text-slate-500 tracking-wider uppercase border-b border-slate-100 pb-2">
                {CINEMA_ROOM_LABELS.seatTools}
              </h3>

              <div className="space-y-4">
                {selectedSeats.length > 0 ? (
                  <div className="flex items-center justify-between p-3 bg-sky-50 border border-sky-100 rounded-xl text-xs font-bold text-sky-800">
                    <span>Đã chọn {selectedSeats.length} ghế</span>
                    <button
                      type="button"
                      onClick={clearSelection}
                      className="px-2.5 py-1 text-[10px] bg-sky-200 hover:bg-sky-300 text-sky-900 rounded font-black transition-colors cursor-pointer"
                    >
                      Bỏ chọn
                    </button>
                  </div>
                ...
                <div className="space-y-2 flex flex-col">
                  <span className="text-[10px] font-black text-slate-400 uppercase">Đổi loại ghế</span>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => applyAction("normal")}
                      disabled={selectedSeats.length === 0 || isSaving}
                      className="py-2 text-xs font-bold bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 rounded-lg transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      disabled={selectedSeats.length === 0 || isSaving}
                      className="py-2 text-xs font-bold border-2 border-amber-500 disabled:opacity-50 text-amber-700 hover:bg-amber-50 rounded-lg transition-colors cursor-pointer"
                    >
                  ...
                  <button
                    ...
                    className="w-full py-2.5 text-xs font-bold bg-purple-600 hover:bg-purple-700 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-lg transition-colors cursor-pointer"
                  >
                    Ghép đôi ({CINEMA_ROOM_LABELS.seatTypeCouple})
                  </button>
                  {couplePlan.error && (
                    <p className="text-[11px] font-bold text-red-600 leading-tight">{couplePlan.error}</p>
                  )}
                  <button
                    onClick={() => applyAction("decouple")}
                    disabled={selectedSeats.length === 0 || isSaving}
                    className="w-full py-2.5 text-xs font-bold border border-purple-200 text-purple-700 hover:bg-purple-50 disabled:opacity-50 rounded-lg transition-colors cursor-pointer"
                  >
                    Hủy ghép đôi
                  </button>
                </div>

                <div className="space-y-2 flex flex-col border-t border-slate-100 pt-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase">Trạng thái ghế</span>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => applyAction("active")}
                      disabled={selectedSeats.length === 0 || isSaving}
                      className="py-2 text-xs font-bold bg-green-50 text-green-700 hover:bg-green-100 disabled:opacity-50 rounded-lg transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      disabled={selectedSeats.length === 0 || isSaving}
                      className="py-2 text-xs font-bold bg-red-50 text-red-700 hover:bg-red-100 disabled:opacity-50 rounded-lg transition-colors cursor-pointer"
                    >
                ...
                <div className="space-y-2 flex flex-col border-t border-slate-100 pt-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase">Lối đi</span>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => applyAction("delete")}
                      disabled={selectedSeats.length === 0 || isSaving}
                      className="py-2 text-xs font-bold bg-slate-800 hover:bg-slate-900 disabled:opacity-50 text-white rounded-lg transition-colors cursor-pointer"
                    >
                      Xóa (Lối đi)
                    </button>
                    <button
                      ...
                      disabled={selectedSeats.length === 0 || isSaving}
                      className="py-2 text-xs font-bold border border-slate-800 text-slate-800 hover:bg-slate-100 disabled:opacity-50 rounded-lg transition-colors cursor-pointer"
                    >
                      Khôi phục
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-3">
              <h4 className="text-[10px] font-black text-slate-400 tracking-wider uppercase">Hướng dẫn thiết lập</h4>
              <ul className="text-xs text-slate-500 space-y-2 list-disc pl-4 font-bold leading-relaxed">
