...
const STATUS_LABEL = {
  0: { text: "Chờ xác thực", cls: "bg-yellow-100 text-yellow-700" },
  1: { text: "Hoạt động",    cls: "bg-green-100 text-green-700" },
  2: { text: "Đã khóa",      cls: "bg-red-100 text-red-600"    },
};
...
const inputCls = "w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 transition";
  ...
  <div className="space-y-4">
    ...
    {!isEdit && (
      <div className="flex items-start gap-2 p-3 bg-indigo-50 rounded-lg">
        ...
        <p className="text-xs text-indigo-700">Tài khoản mới sẽ có mật khẩu mặc định: <strong>Admin@123456</strong></p>
      </div>
    )}
  </div>
);
...
const AdminManagement = () => {
  ...
  // ─── Table helpers ────────────────────────────────────────────────────────
  const thCls = "px-4 py-3 text-xs text-gray-500 font-bold whitespace-nowrap";
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gray-50 font-sans -m-8 md:-m-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <input
              ...
              className="bg-gray-100 text-sm rounded-full pl-10 pr-4 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-300 w-56"
            />
      ...
      {/* Error banner */}
      {errMsg && <div className="mx-8 mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600">{errMsg}</div>}
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                  {paginated.length === 0 ? (
                    ...
                    return (
                      <tr key={admin.accountId} className="hover:bg-gray-50/60 transition-colors">
                        <td className={tdCls}>{rowNum}</td>
                        <td className={tdCls}>
                          <div className="flex items-center gap-3">
                            {admin.image ? (
                              <img src={admin.image} alt="" className="w-8 h-8 rounded-full object-cover border border-gray-200" />
                            ) : (
                        ...
                        <td className={tdCls}>
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`p-1.5 rounded-lg transition-colors ${admin.status === 2 ? "text-gray-400 hover:text-green-600 hover:bg-green-50" : "text-gray-400 hover:text-orange-500 hover:bg-orange-50"}`}
                            >
        ...
        <Modal title="Thêm tài khoản Admin mới" onClose={() => setAddOpen(false)}>
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setAddOpen(false)} className="flex-1 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition">Hủy</button>
      ...
      {editTarget && (
        <Modal title={`Sửa thông tin: ${editTarget.fullName}`} onClose={() => setEditTarget(null)}>
          <form onSubmit={handleEdit} className="space-y-4">
            ...
            {formErr && <p className="text-red-500 text-sm">{formErr}</p>}
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setEditTarget(null)} className="flex-1 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition">Hủy</button>
              <button type="submit" disabled={submitting} className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold transition disabled:opacity-50">
        ...
        <Modal title="Xác nhận đặt lại mật khẩu" onClose={() => setResetTarget(null)}>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg">
              ...
              <p className="text-xs text-blue-700">Mật khẩu mới sẽ là: <strong>Admin@123456</strong></p>
            </div>
            <div className="flex gap-3 pt-2">
              <button onClick={() => setResetTarget(null)} className="flex-1 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition">Hủy</button>
      ...
      {deleteTarget && (
        <Modal title="Xác nhận xóa tài khoản Admin" onClose={() => setDeleteTarget(null)}>
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Bạn có chắc muốn <strong className="text-red-600">xóa vĩnh viễn</strong> tài khoản Admin{" "}
              <strong>{deleteTarget.fullName}</strong>? Hành động này không thể hoàn tác.
            </p>
            <div className="flex gap-3 pt-2">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition">Hủy</button>
