...
public class MemberService {
    ...
    public List<MemberDTO> getAllMembers() {
        ...
        for (Member member : memberListFromDB) {
            ...
            // Quăng thằng Member vào "Cỗ máy Mapper" để nó tự động biến hình thành DTO (Sếp khỏi phải tự Set bằng tay)
    ...
    public MemberDTO updateMember(String memberId, UpdateMemberRequest request) {
        ...
        if (request.getDateOfBirth() != null) {
            ...
            if (java.time.Period.between(request.getDateOfBirth(), today).getYears() < 18) {
    ...
    public void resetPassword(String memberId) {
        ...
        String noiDung = "Chào " + account.getFullName() + ",\n\n"
                + "Yêu cầu khôi phục mật khẩu cho tài khoản của bạn đã được thực hiện bởi Quản trị viên.\n"
                + "Mật khẩu mới của bạn là: " + newPassword + "\n\n"
                + "Vui lòng đăng nhập bằng mật khẩu này và đổi lại mật khẩu mới để đảm bảo an toàn.";
        emailService.sendSimpleEmail(account.getEmail(), tieuDe, noiDung);
    }
    ...
    public MemberDTO createMember(CreateMemberRequest request) {
        ...
        String tieuDe = "Thông tin tài khoản thành viên MTMS Cinema";
        String noiDung = "Chào " + request.getFullName().trim() + ",\n\n"
                + "Tài khoản thành viên của bạn đã được khởi tạo thành công bởi Ban quản trị MTMS Cinema.\n"
                + "Tên đăng nhập: " + request.getUsername().trim() + "\n"
                + "Mật khẩu của bạn là: " + rawPassword + "\n\n"
                + "Tài khoản của bạn đã được kích hoạt. Hãy đăng nhập để sử dụng các dịch vụ của MTMS Cinema.";
