...
const quickActions = [
  ...
  },
  {
    ...
    description: "Tìm hóa đơn đã đặt",
    icon: Search,
  ...
  },
  {
    ...
    description: "Ưu đãi đang áp dụng",
    icon: Tag,
...
const EmployeeDashboard = () => {
  ...
  const fetchOverview = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Không thể tải dữ liệu bảng điều khiển nhân viên.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-[#C00000] dark:text-[#ff4d57]">Hoạt động nhân viên</p>
          <h1 className="mt-2 text-3xl font-black text-gray-950 dark:text-white">Bảng điều khiển vận hành</h1>
          <p className="mt-1 text-sm font-medium text-gray-500 dark:text-white/50">
            {lastUpdated ? `Cập nhật lần cuối ${lastUpdated.toLocaleTimeString("vi-VN")}` : "Đang tải dữ liệu vận hành mới nhất"}
          </p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          icon={CalendarClock}
          label="Trong 2 giờ tới"
          value={`${number(showtimes.length)} suất chiếu`}
          tone="bg-blue-50 text-blue-600 dark:bg-blue-950/40 dark:text-[#4CC9F0]"
        />
        <StatCard
          icon={Bell}
          label="Sắp bắt đầu"
          value={`${number(startsSoonCount)} nhắc nhở`}
      ...
      <div className={loading ? "pointer-events-none opacity-60" : "space-y-6"}>
        <section className="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-[#10131A]/80 dark:backdrop-blur-md p-5 shadow-sm transition-colors duration-300">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-base font-black text-gray-950 dark:text-white">Suất chiếu sắp tới</h2>
              <p className="text-sm font-medium text-gray-500 dark:text-white/50">Trong vòng 2 giờ tới</p>
            </div>
            ...
            <table className="w-full min-w-[860px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-white/10 text-xs font-black uppercase tracking-wide text-gray-400 dark:text-white/40">
                  ...
                  <th className="py-3 pr-4">Suất chiếu</th>
                  <th className="py-3 pr-4">Phòng</th>
                  <th className="py-3 pr-4">Ghế</th>
                  <th className="py-3">Còn trống</th>
                </tr>
              </thead>
              <tbody>
                {showtimes.map((showtime) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 pr-4 font-semibold text-gray-600 dark:text-white/60">{showtime.movieFormat || "Tiêu chuẩn"}</td>
                      ...
                      <td className="py-3 pr-4 font-semibold text-gray-600 dark:text-white/60">
                        {number(showtime.soldSeats)} đã bán / {number(showtime.totalSeats)}
                      </td>
                      <td className="py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-black text-gray-950 dark:text-white">
                              {number(showtime.availableSeats)} / {number(showtime.totalSeats)} còn trống
                            </div>
                            <div className="text-xs font-semibold text-gray-500 dark:text-white/45">
                              {Number(showtime.occupancyRate || 0).toFixed(2)}% đã đặt
                            </div>
                          </div>
                        </div>
                      </td>
                ...
                {!showtimes.length && (
                  <tr>
                    <td colSpan="6" className="py-10 text-center font-semibold text-gray-400 dark:text-white/40">
                      Không có suất chiếu nào trong 2 giờ tới
                    </td>
                  </tr>
                )}
              </tbody>
