...
const EmployeeSelectShowtimePage = () => {
  ...
  const movieIdsWithSchedule = useMemo(
    () => new Set(allSchedules.map((s) => s.movieId)),
    [allSchedules]
  );

  const selectableMovies = useMemo(
    () => activeMovies.filter((m) => movieIdsWithSchedule.has(m.movieId)),
    [activeMovies, movieIdsWithSchedule]
  );
