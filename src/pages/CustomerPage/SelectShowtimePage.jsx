...
const SelectShowtimePage = () => {
  ...
  // Tập hợp các ngày có ít nhất 1 suất chiếu (để hiển thị chấm đánh dấu)
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3.5 overflow-x-auto pb-2 scrollbar-none">
            {days.map((day, idx) => {
              ...
              const hasSchedule = datesWithSchedule.has(day.dateStr);
              return (
                <button
                  key={idx}
                  onClick={() => setSelectedDateStr(day.dateStr)}
                  className={`relative w-[85px] py-4 px-2 rounded-xl flex-shrink-0 flex flex-col items-center justify-center space-y-1 border shadow-sm transition-all cursor-pointer transform active:scale-95 ${
                    isSelected
                ...
                >
                  ...
                  {hasSchedule && (
                    <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-[#E50914]" />
                  )}
                </button>
              );
            })}
