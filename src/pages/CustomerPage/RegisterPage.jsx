  ...
  const handleRegister = async (e) => {
    ...
    if (!formData.dateOfBirth || formData.dateOfBirth.trim() === '') {
      ...
      if (isNaN(dob.getTime()) || dob > today) {
        errorsObj.dateOfBirth = 'Ngày sinh không hợp lệ! (định dạng dd/mm/yyyy)';
      } else if (age < 18) {
    ...
    try {
      ...
      const addressParts = [];
      if (detailAddress?.trim()) addressParts.push(detailAddress.trim());
      if (district?.trim()) addressParts.push(district.trim());
      if (province?.trim()) addressParts.push(province.trim());
      const submitData = { ...rest, address: addressParts.join(', ') };
          ...
          <form className="space-y-7" onSubmit={handleRegister}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Tỉnh / Thành phố" required error={errors.province}>
                    <AppleSelect
                      ...
                      options={Object.keys(locationData).map((p) => ({ value: p, label: p }))}
                    />
                  </Field>
                  <Field label="Quận / Huyện" required error={errors.district}>
                    <AppleSelect
                      ...
                      options={
                        formData.province
                          ? locationData[formData.province].map((d) => ({ value: d, label: d }))
                          : []
                      }
                    />
                  </Field>
            ...
            <div className="flex flex-col items-stretch gap-4 border-t border-black/[0.06] pt-6 dark:border-white/10 sm:items-center">
              <label className="flex max-w-xl cursor-pointer select-none items-start gap-2.5 text-xs font-medium leading-relaxed text-[#6B7280] dark:text-white/50">
                ...
                <span>
                  Tôi đồng ý với{' '}
                  <Link to="/terms" className="font-bold text-[var(--cine-red)] hover:underline">
                    Điều khoản sử dụng
                  </Link>{' '}
                  và{' '}
                  <Link to="/privacy" className="font-bold text-[var(--cine-red)] hover:underline">
                    Chính sách bảo mật
                  </Link>
                </span>
              </label>
