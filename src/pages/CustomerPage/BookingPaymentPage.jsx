...
import { ChevronLeft, Star, Clock, Film, Gift, Coins, Tag, ShoppingBag, Plus, Minus, Check, MapPin, Calendar, Popcorn, CupSoda, Package, TriangleAlert } from 'lucide-react';
...
const BookingPaymentPage = () => {
  ...
  const handleApplyPromo = () => {
    ...
    setPromoError('');
    PromotionService.validateByCode(promoCodeInput.trim(), scheduleId)
      .then((res) => {
        if (res.data.status === 200 && res.data.data) {
          ...
          setPromoError(res.data.message || 'Mã giảm giá không hợp lệ.');
        }
      })
      .catch((err) => {
        console.error('Lỗi áp dụng voucher:', err);
        setAppliedPromotion(null);
        setPromoError(getPromoApiError(err, 'Mã giảm giá không hợp lệ hoặc đã hết hạn.'));
      })
  ...
  const handleConfirmPayment = () => {
    ...
    createPayment(paymentData)
      ...
      .catch((err) => {
        ...
        const isNetwork =
          !err?.response &&
          (err?.code === 'ERR_NETWORK' || /network error|connection refused/i.test(String(err?.message || '')));
        const msg = isNetwork
          ? 'Không kết nối được máy chủ. Vui lòng kiểm tra backend đang chạy rồi thử lại.'
          : getPromoApiError(err, 'Đặt vé thất bại. Vui lòng thử lại sau.');
        setPromoError(msg);
        alert(msg);
      });
  };
        ...
        {items.map((item) => {
          ...
          return (
            <div key={`${concessionTab}_${id}`} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-4 shadow-sm flex flex-col justify-between space-y-3 transition-colors duration-300">
              ...
              {/* Lựa chọn cỡ & giá tiền */}
              <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                {prices.map((p) => {
                  const sizeLabel = p.size === 'NONE' ? 'Mặc định' : `Cỡ ${p.size}`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-250/50 dark:border-gray-800 pb-5 gap-4">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400 font-bold">
                  <span className="inline-flex items-center gap-1"><MapPin size={13} className="text-[#E50914]" /> {schedule.cinemaRoomName}</span>
                  <span>•</span>
                  <span className="inline-flex items-center gap-1"><Calendar size={13} className="text-[#E50914]" /> {showtimeTimeStr} | {showtimeDateStr}</span>
                  <span>•</span>
            ...
            <div className="space-y-4">
              ...
              <div className="flex border-b border-gray-200 dark:border-gray-800 gap-2">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 py-2 px-4 text-xs font-black uppercase border-b-2 transition-all cursor-pointer ${
                    concessionTab === 'combos'
                      ? 'border-[#E50914] text-[#E50914]'
                      : 'border-transparent text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'
                  }`}
                >
                  <Package size={14} /> Gói Combo ({concessionsData.combos.length})
                </button>
                <button
                  type="button"
                ...
                >
                  <Popcorn size={14} /> Bắp Rang ({concessionsData.foods.length})
                </button>
                <button
                  type="button"
                ...
                >
                  <CupSoda size={14} /> Nước Ngọt ({concessionsData.drinks.length})
                </button>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-white dark:bg-gray-900 border border-gray-900 dark:border-gray-100 text-gray-900 dark:text-gray-100 font-extrabold text-xs md:text-sm tracking-wider uppercase rounded-xl hover:bg-gray-900 dark:hover:bg-gray-100 hover:text-white dark:hover:text-gray-900 transition-all transform active:scale-95 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {checkingPromo ? 'Đang kiểm tra...' : 'ÁP DỤNG'}
                </button>
              </div>
              {promoError && (
                <div className="p-3 px-4 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-300 rounded-xl text-xs font-bold flex items-start gap-2">
                  <TriangleAlert size={14} className="mt-0.5 shrink-0" />
                  <span>{promoError}</span>
                </div>
              )}
              {appliedPromotion && !promoError && (
                <div className="p-3 px-4 bg-green-500/10 border border-green-500/20 text-green-700 dark:text-green-400 rounded-xl text-xs font-bold flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="shrink-0 text-[11px] font-bold text-gray-500 hover:text-red-600 transition-colors"
                  >
                    Bỏ mã
                  </button>
                </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200/80 dark:border-gray-800 p-6 rounded-2xl shadow-sm space-y-6 transition-colors duration-300">
                  ...
                  <div className="pl-3 border-l-2 border-red-200 space-y-1.5 text-[11px] text-gray-500 font-semibold">
                    {Object.values(selectedConcessions).map((item) => (
                      <div key={item.key} className="flex justify-between items-center">
                        <span>{item.name} {item.size !== 'NONE' ? `(Cỡ ${item.size})` : ''} x{item.quantity}</span>
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-3.5 rounded-xl border border-gray-200/80 dark:border-gray-700 text-[10px] font-medium text-gray-500 dark:text-gray-400 flex items-start space-x-2">
              <TriangleAlert size={14} className="text-[#E50914] shrink-0 mt-0.5" />
