...
const BookingSeatPage = () => {
  ...
  useEffect(() => {
    ...
    ScheduleService.getScheduleById(scheduleId)
      ...
      .then(([movieRes, seatsRes]) => {
        ...
        // Chỉ khôi phục ghế tab này đã ghi trong sessionStorage.
        // DRAFT của chính account nhưng không thuộc tab này = ghế kẹt sau hủy → nhả luôn.
        ...
        const displaySeats = loadedSeats.map((s) =>
  ...
  useEffect(() => {
    ...
    return subscribeSeatUpdates(scheduleId, (event) => {
      ...
      // User khác vừa giữ ghế ta đã nhả — xóa marker local=0 để không chặn sync sau này
      applied.forEach((u) => {
      ...
      setTabHeldSeatIds((prev) => {
        ...
        applied.forEach((u) => {
          ...
          if (u.bookingStatus === 0 || u.bookingStatus === 1) {
            ...
            // Hold echo tới trước HTTP → gắn tabHeld ngay, tránh vàng
            next.add(id);
          }
        });
  ...
  // Chỉ nhả ghế khi điều hướng SPA sang trang khác (không phải sang thanh toán).
  useEffect(() => {
    if (!scheduleId) return undefined;
    ...
    const timer = setInterval(() => {
      setHoldRemainingMs((prev) => {
        ...
        if (next <= 0) {
          ...
          if (ids.length) {
            rememberLocalSeatState(localSeatStateRef.current, ids, 0);
            BookingService.releaseSeats(scheduleId, ids).catch(() => {});
          }
  ...
  const handleSeatClick = (seat) => {
    ...
    // Bỏ chọn → nhả DRAFT ngay (optimistic UI + HTTP)
    if (isSelected) {
      ...
      BookingService.releaseSeats(scheduleId, groupIds)
        ...
        .catch(() => {
          // Rollback nếu server không nhả được
          alert('Không thể hủy giữ ghế. Vui lòng thử lại.');
          BookingService.getSeatsByScheduleId(scheduleId).then((seatsRes) => {
            const loaded = seatsRes.data.data || [];
            const reconciled = reconcileSeatsWithLocalState(
              loaded,
              localSeatStateRef.current,
              currentAccountId
            );
    ...
    BookingService.confirmBooking(scheduleId, groupIds)
      ...
      .catch((err) => {
        ...
        BookingService.getSeatsByScheduleId(scheduleId).then((seatsRes) => {
          ...
          // Giữ các ghế đã nhả (status 0), chỉ sync ghế đang hold
