...
const ProfilePage = () => {
  ...
  const handleAvatarChange = async (file) => {
    try {
      ...
      // 3. Gọi API cập nhật thông tin của chính mình
      const response = await CustomerService.updateMyProfile(updateData);
      const updatedMember = response.data.data;
              ...
              <div className="space-y-4">
                ...
                {filteredPoints.length > 0 ? (
                  ...
                  <div className="p-10 bg-amber-50/50 border border-dashed border-amber-200 rounded-2xl text-center text-amber-700 font-bold text-sm">
                    Không tìm thấy lịch sử điểm nào trong khoảng thời gian đã chọn.
                  </div>
                )}
              </div>
            )}
