...
const CheckboxGroup = ({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      onChange([...selectedIds, created[idKey]]);
      setNewName("");
      setIsAdding(false);
    } catch (error) {
      setCreateError(
        error?.response?.data?.message || "Không thể thêm mới, vui lòng thử lại."
      );
    } finally {
      setIsCreating(false);
    }
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          ...
          {onCreate && !isAdding && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-full border border-dashed border-gray-300 dark:border-gray-600 text-xs font-semibold text-gray-500 dark:text-gray-400 hover:border-[#C00000] hover:text-[#C00000] transition-colors"
            >
              <Plus size={12} />
              {createLabel}
            </button>
          ...
          {onCreate && isAdding && (
            <div className="flex items-center gap-1">
              <input
                ...
                maxLength={100}
                disabled={isCreating}
                className="px-3 py-1.5 rounded-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800/60 text-xs text-gray-700 dark:text-gray-200 focus:outline-none focus:border-[#C00000] disabled:opacity-60"
              />
              <button
                type="button"
                onClick={handleCreate}
                disabled={isCreating || !newName.trim()}
                className="p-1.5 rounded-full bg-[#C00000] text-white disabled:opacity-50"
                title="Xác nhận"
              >
              ...
              <button
                ...
                disabled={isCreating}
                className="p-1.5 rounded-full border border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 disabled:opacity-50"
                title="Huỷ"
              >
                <X size={12} />
              </button>
...
const MovieFormFields = ({ formData, onChange, onTypeChange, onVersionChange, typeOptions, versionOptions, onCreateType }) => (
