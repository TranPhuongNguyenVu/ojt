...
const ScheduleManagement = () => {
  ...
  const handleMovieSearchChange = (e) => {
    ...
    movieSearchTimeoutRef.current = setTimeout(() => {
      ...
      MovieService.searchMoviesAdmin(keyword)
        .then((res) => {
  ...
  // Same room's schedules, chronologically ordered — lets quickAddSlot below find the
  // event immediately following any given schedule, not just whichever is last overall
  // (a show followed by an empty gap, e.g. after deleting the show that used to fill it,
  // must still offer to add into that gap).
  const sortedSchedulesByRoom = useMemo(() => {
    const map = new Map();
    schedulesByRoom.forEach((list, roomId) => {
      map.set(roomId, [...list].sort((a, b) => new Date(a.startTime) - new Date(b.startTime)));
    });
    return map;
  }, [schedulesByRoom]);

  const DEFAULT_BUFFER = 30; // AddScheduleModal's default bufferTime for a not-yet-created show

  // Shortest duration among movies that could actually be booked into this room — the real
  ...
  const roomMinDuration = (room) => {
    ...
    const compatible = activeMovies.filter((m) => (m.versions || []).some((v) => roomAcceptsVersion(room, v.versionId)));
    return compatible.length ? Math.min(...compatible.map((m) => Number(m.duration) || Infinity)) : Infinity;
  };

  // The next valid start time after `schedule`, or null if no slot remains. Mirrors the
  // backend's two-sided overlap rule (ScheduleService: newStart >= existingEnd+existingBuffer
  // AND newEnd+newBuffer <= nextExistingStart) so the button only appears when a real show
  // could fit — whether that gap is to the following schedule in the room, or (for the day's
  // last schedule) simply within the 08:00–23:00 booking window.
  const quickAddSlot = (schedule) => {
    const base = new Date(schedule.endTime);
    if (isNaN(base.getTime())) return null;
    const buffer = Number(schedule.bufferTime) || DEFAULT_BUFFER;
    const earliestStart = new Date(base.getTime() + buffer * 60000);

    const room = rooms.find((r) => r.cinemaRoomId === schedule.cinemaRoomId);
    const list = sortedSchedulesByRoom.get(schedule.cinemaRoomId) || [];
    const idx = list.findIndex((s) => s.scheduleId === schedule.scheduleId);
    const nextSchedule = idx >= 0 ? list[idx + 1] : undefined;

    if (nextSchedule) {
      const nextStart = new Date(nextSchedule.startTime);
      if (isNaN(nextStart.getTime())) return null;
      const availableMin = (nextStart.getTime() - earliestStart.getTime()) / 60000 - DEFAULT_BUFFER;
      return availableMin >= roomMinDuration(room) ? earliestStart : null;
    }

    // Last schedule of the day in this room: only the 08:00–23:00 start-time rule applies
    // (no upper bound on end time), same as validateScheduleForm enforces on submit.
    if (
      earliestStart.getFullYear() !== base.getFullYear() ||
      earliestStart.getMonth() !== base.getMonth() ||
      earliestStart.getDate() !== base.getDate()
    ) {
      return null;
    }
    const hour = earliestStart.getHours() + earliestStart.getMinutes() / 60;
    return hour <= LATEST_SCHEDULE_HOUR ? earliestStart : null;
  };
  ...
  const handleQuickAddNext = (schedule) => {
    const next = quickAddSlot(schedule);
    if (!next) return;
    const pad = (n) => String(n).padStart(2, "0");
  ...
  const renderBlock = (schedule) => {
    ...
    // "Add next" appears after any schedule with real room behind it — the gap to the
    // following schedule (or, for the day's last show, to 23:00) must fit at least the
    // shortest bookable movie. Covers the case where a middle show was deleted and left
    // a fillable gap, not just the very last block of the day.
    const canQuickAdd = quickAddSlot(schedule) != null;

    return (
      <div
        key={schedule.scheduleId}
        title={isNarrow ? tooltipText : undefined}
        className={`absolute rounded-lg px-2 py-1.5 overflow-visible shadow-sm hover:shadow-md hover:z-20 cursor-pointer transition-shadow group ${
      ...
      >
        ...
        {canQuickAdd && (
          <button
            ...
            className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-1/2 -right-3.5 -translate-y-1/2 w-[26px] h-[26px] rounded-full border-2 border-[#3467e0] bg-white dark:bg-gray-900 text-[#3467e0] flex items-center justify-center shadow z-30"
          >
