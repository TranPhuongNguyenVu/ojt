...
const ScheduleFormFields = ({
  ...
  const timePart = ((formData.startTime || "").split("T")[1] || "").substring(0, 5);
  const timeOptions =
    timePart && !TIME_SLOTS.includes(timePart)
      ? [...TIME_SLOTS, timePart].sort()
      : TIME_SLOTS;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 text-left">
      ...
      <div>
        ...
        <DateTimeInput
          ...
          onChange={(e) => {
            ...
            const nextDate = next.split("T")[0] || "";
            const nextTime = (next.split("T")[1] || "").substring(0, 5);
            const keepTime = timePart || nextTime || "08:00";
            if (!nextDate) {
              onChange({ target: { name: "startTime", value: "" } });
              return;
            }
            onChange({ target: { name: "startTime", value: `${nextDate}T${keepTime}` } });
          }}
        ...
        <AppleSelect
          ...
          onChange={(slot) => {
            if (!slot) {
              ...
              onChange({ target: { name: "startTime", value: `${datePart}T08:00` } });
              return;
            }
            const d = datePart || minScheduleDateTime().split("T")[0];
            onChange({ target: { name: "startTime", value: `${d}T${slot}` } });
          }}
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            readOnly
            value={estimatedEndTime || "— (chọn phim, ngày & giờ)"}
            className="block w-full rounded-lg border border-gray-100 dark:border-gray-800 pl-10 pr-3 py-2 text-sm bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400 cursor-not-allowed"
          />
        </div>
      </div>
      ...
      <div>
        <label className={fieldLabelClass}>Thời gian nghỉ giữa suất chiếu</label>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 pointer-events-none">
            <Clock size={15} />
          </span>
