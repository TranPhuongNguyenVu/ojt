...
export function validateScheduleForm(formData) {
  ...
  if (!formData.startTime) return "Vui lòng chọn ngày và giờ bắt đầu.";
  const timePart = (formData.startTime.split("T")[1] || "").substring(0, 5);
  if (!timePart) return "Vui lòng chọn giờ bắt đầu.";
  ...
  if (formData.bufferTime !== "" && formData.bufferTime != null && Number(formData.bufferTime) < 0)
    return "Thời gian nghỉ giữa suất chiếu không được âm.";
  return null;
}
