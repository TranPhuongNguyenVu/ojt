...
export const translateScheduleReason = (msg) => {
  ...
  if (lower.includes("movie not found")) return "Không tìm thấy phim.";
  if (lower.includes("room not found")) return "Không tìm thấy phòng chiếu.";
  if (lower.includes("schedule not found")) return "Không tìm thấy suất chiếu.";
  if (lower.includes("overlapping showtime in another room"))
    return "Phim này đã có suất chiếu trùng khung giờ ở phòng khác.";
  if (lower.includes("overlaps with existing"))
    return "Trùng giờ với suất chiếu khác trong cùng phòng (đã tính thời gian nghỉ giữa 2 suất).";
  if (lower.includes("overlap") || lower.includes("trùng"))
    return "Trùng giờ với suất chiếu khác trong cùng phòng (đã tính thời gian nghỉ giữa 2 suất).";
  if (lower.includes("inactive room") || (lower.includes("inactive") && lower.includes("room")))
    return "Không thể xếp lịch cho phòng chiếu đang ngừng hoạt động.";
  if (lower.includes("inactive movie") || (lower.includes("inactive") && lower.includes("movie")))
    return "Không thể xếp lịch cho phim đang ngừng chiếu hoặc chưa mở lịch.";
  if (lower.includes("within movie showing range"))
    return "Giờ chiếu nằm ngoài khoảng thời gian công chiếu của phim.";
  if (lower.includes("version is not supported") || lower.includes("định dạng chiếu không hợp lệ"))
    return "Định dạng đã chọn không thuộc phim này.";
  if (lower.includes("does not support the chosen format"))
    return "Phòng chiếu không hỗ trợ định dạng đã chọn.";
  if (lower.includes("at least from tomorrow") || lower.includes("từ ngày mai"))
    return "Giờ chiếu phải từ ngày mai trở đi.";
  if (lower.includes("more than 10 days") || lower.includes("tối đa 10 ngày"))
    return "Chỉ được tạo lịch trước tối đa 10 ngày.";
  if (lower.includes("cannot schedule") && lower.includes("inactive"))
    return "Không thể xếp lịch vì phim hoặc phòng đang ngừng hoạt động.";
...
export const mapScheduleApiError = (error, fallback) => {
  ...
  return translateScheduleReason(msg);
};
