...
import { classifyVersions, computeFormatWeights } from "./autoGenerate/movieFormatSettings";
...
const MAX_SELECTED_MOVIES = 5; // tối đa 5 phim mỗi lần tạo lịch tự động
const HOUR = 60;
...
const defaultForm = () => ({
  ...
  closeTime: "23:00",
  gapCleanup: 30,
  showsPerDayPerRoom: 4,
});

/** Modal shown when clicking an empty cell (or "add next") on the timeline — lets the admin pick movie + version, and adjust the start time, manually. */
const ManualAddModal = ({ manualAddModal, movies, exclusiveRoomsByMovie = {}, minMinute, maxMinute, checkConflict, onClose, onAdd }) => {
  const { room, clickMinute } = manualAddModal;
  const [startMin, setStartMin] = useState(clickMinute);
  const [addError, setAddError] = useState("");
  const exclusiveOwnerId = exclusiveRoomsByMovie[room.cinemaRoomId];
  const compatibleMovies = movies.filter((movie) => {
    if (exclusiveOwnerId && exclusiveOwnerId !== movie.movieId) {
      return false; // Exclusive room reserved for another movie
    }
    return (movie.versions || []).some((v) => roomAcceptsVersion(room, v.versionId));
  });

  const onTimeChange = (raw) => {
    if (!raw) return;
    const snapped = Math.round(timeToMin(raw) / 5) * 5;
    setStartMin(Math.max(minMinute, Math.min(snapped, maxMinute)));
    setAddError("");
  };

  const handlePick = (movie, version) => {
    const ok = onAdd(movie, version, startMin);
    if (!ok) {
      setAddError("Không thể tạo suất chiếu: khung giờ bị trùng hoặc chưa đủ thời gian dọn dẹp với suất khác trong phòng.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div
        className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-sm mx-4 p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold text-gray-800 dark:text-white">Thêm suất chiếu</h3>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
          <span className="font-semibold">{room.cinemaRoomName}</span>
        </p>
        <div className="mb-3">
          ...
          <input
            ...
            min={minToLabel(minMinute)}
            max={minToLabel(maxMinute)}
            onChange={(e) => onTimeChange(e.target.value)}
            className={fieldInputClass}
          />
        </div>
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">Chọn phim và định dạng muốn thêm vào khung giờ này.</p>
        {addError && (
          <p className="flex items-start gap-1.5 text-xs text-red-600 dark:text-red-400 font-semibold bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-500/30 rounded-lg px-3 py-2 mb-3">
        ...
        <div className="space-y-3 max-h-80 overflow-y-auto">
          {compatibleMovies.map((movie) => {
            const compatibleVersions = (movie.versions || []).filter((v) => roomAcceptsVersion(room, v.versionId));
            return (
              <div key={movie.movieId} className="border border-gray-100 dark:border-gray-800 rounded-lg p-3">
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2 truncate" title={movie.movieNameVn}>
                  {movie.movieNameVn}
                  <span className="ml-2 text-xs text-gray-400 font-normal">{movie.duration} phút</span>
                </p>
                <div className="flex flex-wrap gap-2">
                  {compatibleVersions.map((v) => {
                    const conflict = checkConflict(movie.duration, startMin);
                    return (
                      <button
                        key={v.versionId}
                        type="button"
                        disabled={conflict}
                        title={conflict ? "Trùng giờ hoặc chưa đủ thời gian dọn dẹp với suất khác trong phòng" : undefined}
                        onClick={() => handlePick(movie, v)}
                        className="px-3 py-1.5 text-xs font-bold bg-[#3467e0] hover:bg-[#2c56ba] text-white rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-[#3467e0]"
                      >
                        {v.versionName}
                      </button>
                    );
          ...
          {compatibleMovies.length === 0 && (
            <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">
              Không có phim nào phù hợp định dạng phòng này.
            </p>
          )}
        </div>
      </div>
...
const AutoGenerateSchedulePage = () => {
  ...
  // Manual add modal: opened when clicking an empty cell on the timeline
  const [manualAddModal, setManualAddModal] = useState(null); // null | { dateStr, room, clickMinute }
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setRooms(
          allRooms
            .filter((r) => r.status === "ACTIVE" && r.formats?.length)
            .sort((a, b) => a.cinemaRoomName.localeCompare(b.cinemaRoomName, "vi"))
        );
      } catch (error) {
  ...
  const onToggleMovieSelected = (movieId) => {
    ...
    if (willSelect && selectedMovieIds.length >= MAX_SELECTED_MOVIES) return;
    toggleId(selectedMovieIds, setSelectedMovieIds, movieId);
    if (willSelect) {
      expandMovie(movieId);
    } else {
      // Tự động đóng panel cấu hình khi bỏ chọn phim
      setExpandedMovieIds((prev) => prev.filter((id) => id !== movieId));
    }
  };
  ...
  const ensureMovieSettings = (movieId, movie) => (prev) => {
    ...
    // If the movie has only 1 version, auto-enable it regardless of type
    if (versions.length === 1) {
      formatRatios[String(versions[0].versionId)] = 1;
    } else {
      // Classify versions: ratio (2D/3D share a room) → default ON; toggle (4DX/IMAX) → default OFF
      const selectedRooms = rooms.filter((r) => selectedRoomIds.includes(r.cinemaRoomId));
      const { ratioVersionIds } = classifyVersions(movie, selectedRooms);
      versions.forEach((v) => {
        if (ratioVersionIds.has(v.versionId)) {
          formatRatios[String(v.versionId)] = 1;
        }
        // toggle versions are intentionally absent (OFF by default)
      });
      // Fallback: if nothing ended up enabled, seed the first version ON
      if (Object.keys(formatRatios).length === 0 && versions.length) {
        const first = [...versions].sort((a, b) => a.versionId - b.versionId)[0];
        if (first) formatRatios[String(first.versionId)] = 1;
      }
    }
  ...
  const onToggleFormat = (movie, versionId, included) =>
    setMovieSettings((prev) => {
      ...
      // Determine which group this version belongs to
      const selectedRooms = rooms.filter((r) => selectedRoomIds.includes(r.cinemaRoomId));
      const { ratioVersionIds } = classifyVersions(movie, selectedRooms);
      const isRatioVersion = ratioVersionIds.has(versionId);

      // Guard: must keep at least one ratio-group version enabled
      if (!included && isRatioVersion) {
        const activeRatioCount = computeFormatWeights(movie, cur.formatRatios).filter((f) =>
          ratioVersionIds.has(f.versionId)
        ).length;
        if (activeRatioCount <= 1) return seeded;
      }
      ...
      // Rebuild eligible room restriction after format change
      const allActiveIds = [
        ...computeFormatWeights(movie, formatRatios).map((f) => f.versionId),
        ...(movie.versions || [])
          .filter((v) => !ratioVersionIds.has(v.versionId) && Object.prototype.hasOwnProperty.call(formatRatios, String(v.versionId)))
          .map((v) => v.versionId),
      ];
      const roomIds = cur.roomIds.filter((rid) => {
        const room = rooms.find((r) => r.cinemaRoomId === rid);
        return room && allActiveIds.some((vid) => roomAcceptsVersion(room, vid));
      });
      return { ...seeded, [movie.movieId]: { formatRatios, roomIds } };
    });
  ...
  const buildRequest = () => {
    ...
    const movieRoomIds = {};
    // Only send ratios for selected movies (no need to send the entire movie list)
    selectedMovieIds.forEach((movieId) => {
      const movie = movies.find((m) => m.movieId === movieId);
      if (movie) movieRatios[movieId] = effectiveRatio(movie);
  ...
  // roomId -> movieId: rooms exclusively assigned to a particular movie
  const exclusiveRoomsByMovie = useMemo(() => {
    const map = {}; // roomId -> movieId
    selectedMovieIds.forEach((mid) => {
      const roomIds = movieSettings[mid]?.roomIds || [];
      roomIds.forEach((rid) => {
        map[rid] = mid;
      });
    });
    return map;
  }, [selectedMovieIds, movieSettings]);

  // All currently-schedulable movies compatible with a room's formats (and not reserved
  // for another movie via an exclusive room), regardless of generation scope — the same
  // set ManualAddModal offers when manually placing a show. Used both for the click-to-add
  // picker and to know the shortest movie that could possibly fill a gap.
  const compatibleMoviesForRoom = (room) => {
    const exclusiveOwnerId = exclusiveRoomsByMovie[room.cinemaRoomId];
  ...
  // Shortest duration among movies that could actually be placed in this room — the real
  // lower bound for "is there room after this show". Infinity when nothing is compatible,
  // so a gap never reads as fillable when no movie could ever go there.
  const roomMinDuration = (room) => {
    const compatible = compatibleMoviesForRoom(room);
    return compatible.length ? Math.min(...compatible.map((m) => Number(m.duration) || Infinity)) : Infinity;
  };
  ...
  const eligibleMoviesForRoom = (room) => {
    ...
    return scopeMovies
      ...
      .flatMap((movie) => {
        const { ratioVersionIds } = classifyVersions(movie, selectedRooms);
        const fmtRatios = movieSettings[movie.movieId]?.formatRatios;

        // Ratio group: use percent weights
        const ratioEntries = computeFormatWeights(movie, fmtRatios)
        ...
        // Toggle group: include if ON (key present) and room supports the version
        const toggleEntries = (movie.versions || [])
          .filter(
            (v) =>
              !ratioVersionIds.has(v.versionId) &&
              Object.prototype.hasOwnProperty.call(fmtRatios ?? {}, String(v.versionId)) &&
              roomAcceptsVersion(room, v.versionId)
          )
          .map((v) => ({
            movie,
            versionId: v.versionId,
            versionName: v.versionName,
            ratio: effectiveRatio(movie) * 100, // treat toggle as full-weight
          }));

        return [...ratioEntries, ...toggleEntries];
      })
  ...
  const rows = useMemo(() => {
    ...
    return activeRooms.map((room) => {
      ...
      const gap = Number(form.gapCleanup);
      // Real lower bound for the gap after each event: not just "was this the last
      // block", but "does the space up to the next event (or closing time) actually fit
      // the shortest schedulable movie for this room, plus cleanup buffer both sides" —
      // the same two-sided rule the backend enforces (see ScheduleService overlap check).
      const minDuration = roomMinDuration(room);
      return {
        room,
        ...
        events: events.map((ev, idx) => {
          ...
          const earliestStart = ev.startMin + ev.runtime + gap;
          const canQuickAdd = nextEv
            ? nextEv.startMin - gap - earliestStart >= minDuration
            : earliestStart <= baseAxis.closeMin;
          return {
            idx,
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [baseAxis, activeDateStr, activeRooms, customEvents, preview, movies, visibleMovies, exclusiveRoomsByMovie, form.gapCleanup]);
  ...
  const handleTrackClick = (e, dateStr, room) => {
    ...
    if (isCellSkipped(dateStr, room.cinemaRoomId)) return;

    if (!compatibleMoviesForRoom(room).length) return;

    // Compute click minute on timeline
    const rect = e.currentTarget.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const raw = timeline.openMin + x / PX_PER_MIN;
    const snapped = Math.round(raw / 5) * 5;
    const clickMinute = Math.max(timeline.openMin, Math.min(snapped, timeline.closeMin));

    setManualAddModal({ dateStr, room, clickMinute });
  };
  ...
  // Opens the manual-add picker prefilled with the slot right after this event's cleanup gap,
  // instead of blindly duplicating the same movie — lets the admin pick a different movie/version too.
  const handleQuickAddNext = (e, dateStr, room, ev) => {
    e.stopPropagation();
    if (!timeline) return;
    const start = Math.min(timeline.closeMin, ev.startMin + ev.runtime + Number(form.gapCleanup));
    setManualAddModal({ dateStr, room, clickMinute: Math.max(timeline.openMin, start) });
  };

  // A required cleanup gap must separate consecutive shows in the same room, not just avoid raw time overlap.
  const wouldOverlapGap = (dateStr, room, startMin, runtime) => {
    const gap = Number(form.gapCleanup);
    return getEffectiveEvents(dateStr, room).some(
      (ev) => startMin < ev.startMin + ev.runtime + gap && startMin + runtime + gap > ev.startMin
    );
  };

  // Handle placing a manually chosen movie+version into the timeline. Returns false (and leaves
  // the modal open) when the slot turns out to be invalid, so the caller can surface why.
  const handleManualAdd = (movie, version, startMin) => {
    if (!manualAddModal) return false;
    const { dateStr, room } = manualAddModal;
    const runtime = movie.duration;
    if (wouldOverlapGap(dateStr, room, startMin, runtime)) return false;
    const base = getEffectiveEvents(dateStr, room);
    const goldenRule = goldenRuleAt(dateStr, startMin);
  ...
  const handleSave = async () => {
      ...
      activeRooms.forEach((room) => {
        getEffectiveEvents(d.date, room).forEach((ev) => {
          candidates.push({
            ...
            goldenHourExtra: ev.golden ? ev.goldenExtra ?? null : null,
            insertedIntoBusyDay: !!ev.insertedIntoBusyDay,
            accepted: true,
          });
        });
    ...
    setIsSaving(true);
    try {
      const res = await ScheduleService.autoGenerateConfirm({ candidates, allowInsertIntoBusyDayRoomIds: allowInsertRoomIds });
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <DateInput
                    ...
                    className={fieldInputClass}
                    showIcon={false}
                    title="Từ ngày phải sau hôm nay ít nhất 1 ngày"
                  />
                </div>
                <div>
                  ...
                  <DateInput
                    ...
                    className={fieldInputClass}
                    showIcon={false}
                    title="Đến ngày tối đa cách Từ ngày 6 ngày"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className={fieldInputClass}
                    title="Trong khoảng 08:00–11:00"
                  />
                </div>
                <div>
                  <label className={fieldLabelClass}>Bắt đầu suất cuối (muộn nhất)</label>
                  <input
                    type="time"
                    value={form.closeTime}
                    onChange={(e) => updateField("closeTime", e.target.value)}
                    className={fieldInputClass}
                    title="Suất không được bắt đầu sau giờ này · Nếu giờ kết thúc vượt 00:00 sẽ không được tạo"
                  />
                </div>
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    className={fieldInputClass}
                    title="Khoảng cách giữa suất trước và suất sau trong cùng phòng (đã gồm dọn dẹp + quảng cáo/trailer)"
                  />
                </div>
                <div className="col-span-2">
                  <label className={fieldLabelClass}>Suất chiếu tối đa / phòng / ngày</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                    value={form.showsPerDayPerRoom}
                    onChange={(e) => updateField("showsPerDayPerRoom", Math.min(10, Math.max(1, Number(e.target.value) || 1)))}
                    className={fieldInputClass}
                    title="Số suất chiếu tối đa mỗi phòng có thể xếp trong 1 ngày"
                  />
                </div>
              </div>
            </div>
            ...
            <div className={`${cardClass} flex flex-col h-[600px]`}>
              <h2
                className="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b border-gray-100 dark:border-gray-800 pb-2 mb-4"
                title="Phải chọn ít nhất 1 phim và 1 phòng mới tạo được lịch. Mỗi phim chỉ chạy 1 phiên bản mỗi lượt tạo lịch; tỉ lệ quyết định số suất được chia trong mỗi khung giờ."
              >
                Phạm vi phim &amp; phòng
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 min-h-0">
                <div className="flex flex-col min-h-0">
                  <p className={fieldLabelClass}>
                    Phim (đã chọn {selectedMovieIds.length} / tối đa {MAX_SELECTED_MOVIES})
                    {selectedMovieIds.length >= MAX_SELECTED_MOVIES && (
                      <span className="ml-2 text-amber-500 dark:text-amber-400 normal-case font-semibold">— đã đạt giới hạn</span>
                    )}
                  </p>
                      ...
                      <div key={m.movieId}>
                        <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 py-0.5">
                          <input
                            ...
                            disabled={!selectedMovieIds.includes(m.movieId) && selectedMovieIds.length >= MAX_SELECTED_MOVIES}
                            onChange={() => onToggleMovieSelected(m.movieId)}
                            className="accent-[#C00000] flex-none disabled:opacity-40 disabled:cursor-not-allowed"
                          />
                          <button
                            type="button"
                        ...
                        {expandedMovieIds.includes(m.movieId) && (
                          <div className="mt-1 mb-2 ml-5">
                            <MovieFormatSettingsPanel
                              ...
                              exclusiveRoomsByMovie={exclusiveRoomsByMovie}
                            />
                          </div>
                        )}
                      </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {ev.canQuickAdd && (
                                    <button
                                      type="button"
                                      onClick={(e) => handleQuickAddNext(e, activeDateStr, row.room, ev.raw)}
                                      title="Thêm suất kế tiếp"
      ...
      {manualAddModal && (
        <ManualAddModal
          ...
          exclusiveRoomsByMovie={exclusiveRoomsByMovie}
          minMinute={timeline.openMin}
          maxMinute={timeline.closeMin}
          checkConflict={(runtime, startMin) =>
            wouldOverlapGap(manualAddModal.dateStr, manualAddModal.room, startMin, runtime)
          }
          onClose={() => setManualAddModal(null)}
