...
const CinemaRoomDetail = () => {
  ...
  const applyAction = (action) => {
    if (room?.hasUpcomingSchedules) {
      setErrorMessage(CINEMA_ROOM_LABELS.editLockedBySchedules);
      return;
    }
  ...
  const editLocked = Boolean(room.hasUpcomingSchedules);
  const toolsDisabled = isSaving || editLocked;
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 dark:bg-gray-950 font-sans -m-8 md:-m-10 transition-colors duration-300">
      <header className="h-16 bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 flex items-center justify-between px-8">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            title={editLocked ? CINEMA_ROOM_LABELS.editLockedBySchedules : undefined}
            className="flex items-center gap-1.5 border border-slate-200 dark:border-gray-700 hover:border-[#C00000] hover:text-[#C00000] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:border-slate-200 dark:disabled:hover:border-gray-700 disabled:hover:text-slate-700 dark:disabled:hover:text-gray-300 bg-white dark:bg-gray-900 text-slate-700 dark:text-gray-300 text-sm font-bold px-4 py-2 rounded-lg transition-colors cursor-pointer"
          >
            <Edit size={16} />
          ...
          <button
            ...
            title={editLocked ? CINEMA_ROOM_LABELS.editLockedBySchedules : undefined}
            className="flex items-center gap-1.5 bg-[#C00000] hover:bg-[#a00000] disabled:bg-slate-400 disabled:cursor-not-allowed text-white text-sm font-bold px-5 py-2 rounded-lg transition-colors shadow-sm cursor-pointer"
          >
            <Save size={16} />
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-6">
        ...
        {editLocked && (
          <div className="rounded-lg border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-950/40 px-4 py-3 text-sm text-amber-800 dark:text-amber-300 font-bold flex items-center gap-2">
            <AlertCircle size={18} />
            {CINEMA_ROOM_LABELS.editLockedBySchedules}
          </div>
        )}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-200 dark:border-gray-800 shadow-sm p-6 flex flex-wrap justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={isTogglingStatus || (isActive && editLocked)}
              title={isActive && editLocked ? CINEMA_ROOM_LABELS.editLockedBySchedules : undefined}
              className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
                isActive
                ...
                <div className="space-y-2 flex flex-col">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      disabled={selectedSeats.length === 0 || toolsDisabled}
                  ...
                  <button
                    ...
                    disabled={selectedSeats.length < 2 || toolsDisabled || !!couplePlan.error}
