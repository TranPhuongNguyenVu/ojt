...
import org.example.be.service.AccountEmailService;
...
public class AuthController {
    ...
    public ResponseEntity<?> verifyAndChangePassword(@Valid @RequestBody VerifyAndChangePasswordRequest request) {
    ...
    public ResponseEntity<?> checkEmail(@RequestParam String email) {
        ...
        accountEmailService.sendNotification(
                ...
                account.getFullName(),
                List.of("Vui lòng sử dụng mã OTP bên dưới để hoàn tất xác thực tài khoản của bạn."),
                null,
                "MÃ OTP CỦA BẠN",
                otpCode,
    ...
    public ResponseEntity<?> requestChangePassword(@Valid @RequestBody ChangePasswordRequest request) {
        Account account = accountRepository.findByEmail(request.getEmail().trim())
        ...
        accountEmailService.sendNotification(
                ...
                "Mã xác nhận thay đổi mật khẩu tài khoản ELITE Cinema",
                "ĐỔI MẬT KHẨU",
                "Xác nhận thay đổi mật khẩu",
                account.getFullName(),
                List.of("Bạn đang thực hiện yêu cầu thay đổi mật khẩu. Vui lòng sử dụng mã OTP bên dưới để xác nhận."),
                null,
                "MÃ OTP CỦA BẠN",
                otpCode,
                "Mã này có hiệu lực trong 10 phút. Tuyệt đối không chia sẻ mã này cho bất kỳ ai!");
    ...
    public ResponseEntity<?> confirmChangePassword(@Valid @RequestBody VerifyAndChangePasswordRequest request) {
        String otpKey = request.getEmail().trim().toLowerCase() + "_CHANGE_PASSWORD";
