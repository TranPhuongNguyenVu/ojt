...
public interface TicketRepository extends JpaRepository<Ticket, Integer> {
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query("UPDATE Ticket t SET t.status = org.example.be.enums.TicketStatus.CANCELLED, "
            + "t.cancelledAt = :cancelledAt, t.cancelReason = :cancelReason, "
            + "t.cancelledBy = :cancelledBy, t.cancelSource = :cancelSource, t.scoreRefunded = :scoreRefunded "
            + "WHERE t.invoiceId = :invoiceId AND t.status = org.example.be.enums.TicketStatus.BOOKED")
    int markCancelledByInvoiceId(@Param("invoiceId") Integer invoiceId, @Param("cancelledAt") LocalDateTime cancelledAt,
            @Param("cancelReason") String cancelReason, @Param("cancelledBy") String cancelledBy,
            @Param("cancelSource") String cancelSource, @Param("scoreRefunded") Integer scoreRefunded);
