...
public class DrinkService {
    ...
    @Transactional(readOnly = true)
    public DrinkDTO getPublicById(Integer id) {
        org.example.be.entity.Drink drink = findOrThrow(id);
        if (CUSTOMER_HIDDEN_STATUSES.contains(drink.getStatus())) {
            throw new jakarta.persistence.EntityNotFoundException("Không tìm thấy thức uống.");
        }
    ...
    public DrinkDTO delete(Integer id) {
        ...
        if (drink.getStatus() == ConcessionStatus.DELETED) {
            throw new EntityNotFoundException("Không tìm thấy thức uống.");
        }
    ...
    private Drink findOrThrow(Integer id) {
        return drinkRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Không tìm thấy thức uống."));
    }
    ...
    private void requireUniqueName(String name, Integer excludeId) {
        ...
        if (duplicate) {
            throw new IllegalArgumentException("Tên thức uống đã tồn tại.");
        }
    }
