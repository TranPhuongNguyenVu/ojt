    ...
    private List<ComboItem> buildItems(List<ComboItemRequestDTO> itemRequests) {
        ...
        for (ComboItemRequestDTO req : requests) {
            ...
            if (req.getQuantity() == null || req.getQuantity() <= 0) {
                throw new IllegalArgumentException("Combo item quantity must be greater than 0");
            }
            if (req.getSize() == null) {
                throw new IllegalArgumentException("Combo item size is required");
            }
            ...
            if (hasFoodId) {
                ...
                Drink drink = drinkRepository.findById(req.getDrinkId())
                        .orElseThrow(() -> new IllegalArgumentException("Drink not found: " + req.getDrinkId()));
