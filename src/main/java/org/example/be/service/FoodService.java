...
public class FoodService {
    ...
    @Transactional(readOnly = true)
    public FoodDTO getPublicById(Integer id) {
        Food food = findOrThrow(id);
        if (CUSTOMER_HIDDEN_STATUSES.contains(food.getStatus())) {
            throw new EntityNotFoundException("Không tìm thấy món ăn.");
        }
    ...
    private Food findOrThrow(Integer id) {
        return foodRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Không tìm thấy món ăn."));
    }
