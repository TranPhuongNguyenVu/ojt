...
public class TicketEmailService {
    ...
    private void sendTicketEmail(Integer invoiceId) {
        ...
        double subtotalMoney = computeSubtotal(detail);
        double discountAmount = Math.max(0, subtotalMoney - detail.getTotalMoney());
        context.setVariable("totalMoney", detail.getTotalMoney());
        context.setVariable("discountAmount", discountAmount);
        context.setVariable("subtotalMoney", subtotalMoney);
    ...
    private double computeSubtotal(TicketDetailDTO detail) {
        double seatSubtotal = detail.getSeats().stream()
                .mapToDouble(TicketDetailDTO.SeatInfoDTO::getPrice)
                .sum();
        double concessionSubtotal = detail.getConcessions().stream()
                .map(ConcessionLineDTO::getLineTotal)
                .filter(Objects::nonNull)
                .mapToDouble(BigDecimal::doubleValue)
                .sum();
        return seatSubtotal + concessionSubtotal;
    }
